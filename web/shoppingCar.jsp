...
<div class="mianCont">
    ...
    <div class="cont">
        ...
        <table class="orderList">
            ...
            <%
                Shopping_Car shopping_car=(Shopping_Car) request.getAttribute("ShoppingCar");
                if(shopping_car==null ||shopping_car.getOrder_merchandises().size()==0){
            %><tr><td colspan="6">购物车中暂时没有任何商品！</td></tr>
            <tr>
                <%}
                else{
                    i=0;
                    while (i<shopping_car.getOrder_merchandises().size()){
                %>
            <tr>
                <td><input type="checkbox" name="idmerchandise" value="<%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getIdmerchandise()+"+"+shopping_car.getOrder_merchandises().get(i).getQuantity()%>"/></td>
                <td>
                    <dl>
                        <dt><a href="Merchandise?type=idmerchandise&&idmerchandise=<%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getIdmerchandise()%>"><img src="upload/<%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getPhoto()%>" width="85" height="85" /></a></dt>
                        <dd><%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getName()%></dd>
                        <div class="clears"></div>
                    </dl>
                </td>
                <td><strong class="red">￥<%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getPrice()%></strong></td>
                <td>
                    <div class="jia_jian">
                        <%=shopping_car.getOrder_merchandises().get(i).getQuantity()%>
                    </div>
                </td>
                <td><strong class="red">￥<%=shopping_car.getOrder_merchandises().get(i).getSum()%></strong></td>
                <td>
                    <a href="ShoppingCar?type=delete&&idmerchandise=<%=shopping_car.getOrder_merchandises().get(i).getMerchandise().getIdmerchandise()%>&&pageLabel=shoppingCar" class="green">删除</a>
                </td>
            </tr>
            <%
                    i++;    }}
            %>
            <tr>
                <td colspan="6"><div class="shanchu"><a href="ShoppingCar"><img src="images/lajio.jpg" /> 全部删除</a></div></td>
            </tr>
        </table><!--orderList/-->
        <div class="jiesuan">
